...
io.on("connection", function (socket) {
  ...
  socket.on("SignalUploaded", async function () {
    ...
    const userNotifications = await NotificationModel.find({}).sort({
      createdAt: -1,
    });
  ...
  socket.on('sendMessage', async function(newMessage,token) {
    ...
    if(!token){
        const messageData = {
            message: newMessage, // Use newMessage, which is passed in from the client
            role: 'User', // Assuming the role is User, change as needed
            id: null, // Set the appropriate user ID if available, for now, it's null
            pic: "https://icon-library.com/images/anonymous-avatar-icon/anonymous-avatar-icon-25.jpg" // Default pic
        };
    
        const result = new UserMessageSchemas(messageData); // Pass structured message object
        await result.save();
        console.log(result);
        socket.broadcast.emit('receiveMessage', newMessage); // This will send the message back to all connected clients
    ...
    const messageData = {
        message: newMessage, // Use newMessage, which is passed in from the client
        role: decode.role || 'User', // Assuming the role is User, change as needed
        id: null, // Set the appropriate user ID if available, for now, it's null
        pic: "https://icon-library.com/images/anonymous-avatar-icon/anonymous-avatar-icon-25.jpg" // Default pic
    };
    ...
    // Ensure that the message is correctly structured according to your schema
   
    // Emit back to the client to update the UI
    socket.broadcast.emit('receiveMessage', newMessage); // This will send the message back to all connected clients
});
